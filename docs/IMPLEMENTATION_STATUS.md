...
- The modeless setup guide now attaches action callbacks to its shell window and routes setup-stage commands through the shared `CommandRouter`; implemented modeless actions such as close update the shared `WindowRegistry`, while deferred setup capture commands return structured unavailable results.
