...
class ModeDefinition:
    ...
    def from_mapping(cls, data: Mapping[str, Any]) -> ModeDefinition:
        ...
        from metrology_process_planner.domains.modes.mode_definition_io import (
...
class ModeRegistry:
    ...
    def visible_definitions(self) -> tuple[ModeDefinition, ...]:
        """Return definitions intended for normal operator selection."""

        return tuple(definition for definition in self._definitions if definition.visible)

    def visible_mode_ids(self) -> tuple[str, ...]:
        """Return visible definition ids in declaration order."""

        return tuple(definition.mode_id for definition in self.visible_definitions())
...
def built_in_mode_registry() -> ModeRegistry:
    ...
    from metrology_process_planner.domains.modes.mode_builtins import (
