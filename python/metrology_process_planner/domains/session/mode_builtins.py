...
from metrology_process_planner.domains.session.mode_registry import ModeDefinition, ModeRegistry
...
PROCESS_EDITOR_ACTIONS = (
    "composite_save",
    "composite_retake_inner",
    "composite_retake_parent",
    "composite_discard",
    "composite_exit",
    "attach_recipe",
...
def _core_modes() -> tuple[ModeDefinition, ...]:
    return (
        _mode(SessionMode.SIMPLE_CAPTURE, "Simple Capture", supports_measurements=True),
        _mode(SessionMode.FAST_BATCH_CAPTURE, "Fast Batch Capture"),
        _mode(SessionMode.CAD_REVIEW, "CAD Review"),
        _mode(SessionMode.OPTICAL_METROLOGY, "Optical Metrology", ("site_box", "point")),
        _mode(SessionMode.CDSEM_CAPTURE, "CDSEM Capture", supports_measurements=True),
        _mode(SessionMode.GRID_MEASUREMENT, "Grid Measurement", ("site_box", "grid")),
        _mode(
            SessionMode.PROCESS_AWARE_METROLOGY,
            "Process-Aware Metrology",
            supports_measurements=True,
        ),
        _mode(SessionMode.PROCESS_FLOW_SUMMARY, "Process Flow Summary"),
    )
...
def _profilometry_mode() -> ModeDefinition:
    return ModeDefinition(
        ...
        process=ProcessPolicy("recommended", "line_profile", "default_cross_section"),
...
def _ellipsometry_mode() -> ModeDefinition:
    return ModeDefinition(
        ...
        process=ProcessPolicy("recommended", "point_stack", "default_point_stack"),
...
def _mode(
    ...
    return ModeDefinition(
        ...
        capabilities=ModeCapabilities(supports_measurements=supports_measurements),
        capture=CaptureSequenceDefinition(supported_primitives=primitives),
...
def _process_artifacts(annotation_role: str, *process_roles: str) -> ArtifactPolicy:
    return ArtifactPolicy(
        (
            ...
            *(
                ArtifactOutputDefinition("process_output", role)
...
def _process_editor(previews: tuple[str, ...]) -> EditorPolicy:
    return EditorPolicy(PROCESS_EDITOR_GROUPS, previews, PROCESS_EDITOR_ACTIONS)
