"""Standalone line capture commit helpers for canvas interaction."""
...
def commit_pending_line(
    session: SessionRecord,
    context: InteractionContext,
    start: Point,
    end: Point,
) -> InteractionResult:
    """Commit a parentless line gesture into a pending capture."""

    pending_id = _next_pending_id(session)
    canvas_id = next_id("canvas", (item.id for item in session.canvas_objects))
    geometry = CaptureGeometry.line(start, end)
