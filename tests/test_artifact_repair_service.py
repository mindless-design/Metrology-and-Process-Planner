...
class ArtifactRepairServiceTests(unittest.TestCase):
    def test_unavailable_repair_preserves_status_for_missing_dependency(self) -> None:
        target = replace(
            make_artifact("measurement"),
        ...
        source = session(artifacts={"measurement": target})
    ...
    def test_declaration_without_handler_is_unavailable_for_repair(self) -> None:
        target = replace(
            make_artifact("crop"),
            status=ArtifactStatus.MISSING,
    ...
    def test_repair_unavailable_when_source_layout_required(self) -> None:
        ...
        source = session(artifacts={"crop": target})
    ...
    def test_superseded_artifact_does_not_block_report_readiness(self) -> None:
        source = session(
            artifacts={"old": replace(make_artifact("old"), status=ArtifactStatus.SUPERSEDED)}
        )

        scanned, result = ArtifactScanner().scan_session(source, temp_paths())

        self.assertEqual(ArtifactStatus.SUPERSEDED, scanned.artifacts["old"].status)
    ...
    def test_artifact_relink_updates_path_and_clears_missing_warning(self) -> None:
        ...
        source = session(artifacts={"crop": make_artifact("crop")})
    ...
    def test_artifact_registry_round_trips_through_session_json(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (paths.folder / target.relative_path).write_text("image", encoding="utf-8")
    ...
    def test_repair_routes_to_registered_generator(self) -> None:
        def handler(_session, artifact, _paths):
            return replace(artifact, status=ArtifactStatus.PRESENT)

        registry = ArtifactGeneratorRegistry(
            (GeneratorRegistration("fake", ("captured_site_image",), handler=handler),)
        )
        target = replace(
            make_artifact("crop"),
