...
class SessionEditorShellControllerTestsPart1(unittest.TestCase):
    def test_shell_renders_generic_sections_and_callbacks(self) -> None:
        ...
        self.assertNotIn("Setup", [label for label, _value in window["header"]])
        self.assertIn(("Capture", "idle"), window["header"])
        self.assertIn(("Selected", "Demo (ready)"), window["header"])
        self.assertNotIn(
            "Process Context",
            [label for label, _value in window["header"]],
        )
