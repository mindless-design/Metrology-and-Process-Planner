"""KLayout batch-mode probe script builders."""
...
def line_capture_adapter_script() -> str:
    """Return a probe script for line capture adapter behavior in KLayout."""

    return textwrap.dedent(_LINE_CAPTURE_ADAPTER_SCRIPT)


def profilometry_line_capture_adapter_script() -> str:
    """Return a probe script for profilometry compound line capture in KLayout."""

    return textwrap.dedent(_PROFILOMETRY_LINE_CAPTURE_ADAPTER_SCRIPT)


_LINE_CAPTURE_ADAPTER_SCRIPT = """
import pya
...
print("MEASUREMENT_ID=" + adapter.session.captures[0].measurements[0].id)
print("LAYOUT_UNCHANGED=" + str(before == after))
print("OVERLAY_CANVAS=" + str("canvas-001" in overlay_ids))
"""


_PROFILOMETRY_LINE_CAPTURE_ADAPTER_SCRIPT = """
import pya
from metrology_process_planner.domains.geometry import Box
from metrology_process_planner.domains.session import (
    CanvasObject,
...
session = SessionRecord(
    "session-001",
    "Demo",
    SessionMode.PROFILOMETRY_PLANNER,
    "2026-06-24T00:00:00Z",
    "2026-06-24T00:00:00Z",
    canvas_objects=(canvas,),
    pending_captures=(pending,),
)
session = arm_inner_feature_capture(session, "pending-001", profilometry_request())
backend = KLayoutOverlayBackend(lambda command: ("marker", command.object_id))
adapter = KLayoutCaptureGestureAdapter(session, CanvasOverlayManager(backend))
...
overlay_ids = {command.object_id for command in backend.commands}
print("PROFILE_LINE_RELEASED=" + str(released.handled))
